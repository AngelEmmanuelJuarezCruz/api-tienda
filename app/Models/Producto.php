...
class Producto extends Model
{
    ...
    public function scopeProximosACaducar($query, $dias = 7)
    {
        ...
        return $query->whereHas('lotesProducto', function ($query) use ($fecha) {
            $query->whereNotNull('fecha_caducidad')
                  ->whereBetween('fecha_caducidad', [now(), $fecha]);
        });
    }

    public function getFechaCaducidadAttribute()
    {
        if ($this->relationLoaded('lotesProducto')) {
            return $this->lotesProducto
                ->whereNotNull('fecha_caducidad')
                ->sortBy('fecha_caducidad')
                ->first()?->fecha_caducidad;
        }
